...
var startURL = "https://www.proxyscan.io/api/proxy?last_check=9800&uptime=35&limit=1&level=anonymous&type=socks4,socks5";
var pingURL = "&ping=";
var regURL = "&country=";
var ip;
var port;
...
function setUserAgent(checkbox){
    var checkboxes = document.getElementsByName('checkUA');
    checkboxes.forEach((item) => {
        if (item !== checkbox) item.checked = false
    });
    UA = checkbox.value;
...
const generate =()=>{
    if(reg != null){
        URL = startURL + regURL + reg + pingURL + ping;
    }else{
        URL = startURL + pingURL + ping;
    }
    console.log(URL)
    fetch(URL)
...
const startMultiple =async()=>{
    var counts = document.getElementById("jumlahChild").value;
    console.log(counts);
